# PluginLab 阶段 2-5 开发计划（Momus 审查定稿 v1）

> 2026-08-03 制定。草案经 Momus（Plan Critic）严格审查（基于真实代码核验），修正后定稿。
> 用户确认后开始执行。每阶段：TDD + 实测验证 + push GitHub main。
...
草案没回答"新 sweep 命令？"。**方案**：新增 `scan` IPC 命令（`{type, param_id, values[]（归一化 0-1）, path}`），响应 `{"ok":true,"runs":N,"export_path":...}`；内部**复用 `MeasurementSession::run()`**（每轮 setParam + setType + run + 分析 + 收集）；**不改动现有 measure 响应格式**（向后兼容）。`MeasurementSession::run()` 的 switch 目前同时管"信号类型"与"分析类型"——阶段 2.4 先拆为"source 决定信号、type 决定分析"二维，扫描才能组合。
...
DESIGN §8.5 已定案但草案没落实：扩展为 `blockCallback(float progress, const dryBlock&, const wetBlock&)`，每块调用；GR 累加在回调内做（每块 2 次 RMS，微秒级），UI 刷新走 `AsyncUpdater` + 50ms 节流。现有 `SweepRunnerTests` 只测 cancel，扩展签名兼容成本低。

**P1-9：attack/release 估计算法需具体化**（阶段 4.2）
"从 GR 时间线提取时间常数"不可执行。**方案**：
...
DESIGN §8.1 明确"人声特有处理只能真实 vocal 触发"，需要 vocal 专用分析（干/湿 RMS 包络时间线、逐帧 FFT 频谱时间线）。草案把 FilePlayback 当"又一个信号源"，没说 vocal 进来**分析什么**。§8.2 的 `AnalysisStrategy`（按 sourceType 分流）草案丢了。
...
- **P2-13：与 §8 定稿方案脱节**：§8.2 定稿 RecorderEngine/ParameterTimeline/AnalysisStrategy/WavExporter + "SweepRunner 不动"边界。草案只留 FilePlayback。**明确**：AnalysisStrategy 在阶段 4 引入；RecorderEngine 作为扫描协调器在阶段 5；WavExporter（用户要听干/湿对比）与 ParameterTimeline（记录模式参数自动化）**显式延后**，在 DESIGN.md 记录该决定，避免与 §8 冲突。
...
- **测试**：动态生成 1s 48k stereo wav（`juce::WavAudioFormat`）→ 读样本数正确、reset 重放一致、mono→stereo 映射、44.1k 文件重采样时长正确
...
- GUI：输入源下拉 + 文件选择器（`FileChooser`）+ 噪声/动态参数面板
- **测试**：CommandParserTests 覆盖 source 字段分支；GUI 手测
- **验收**：IPC 用 file 源跑通 vocal 全流程；GUI 源可切换

**阶段 2 验收**：33 测试全绿 + 新增用例；vocal 全流程可用；JSON 含 source 元数据。
...
- 流程：快照全部参数 → for each value：`setParam` → 让出 20ms（P1-7）→ `run()` → **重读 latency（P1-5）** → 收集 `{value, latency, result}` → 恢复快照（RAII）
...
- **验收**：Pro-Q 4 单参数 10 档扫描 < 60s；JSON 可被 Python 直接解析
...
- **验收**：Pro-Q 4 单参数 10 档扫描 < 60s；GUI 10 条曲线可分辨；JSON 可被 Python 直接解析
...
**阶段 4 验收**：TestCompressorPlugin 全链路；动态信号 GR 时间线 τ 误差 <10%；Pro-C 3 实测 GR 曲线合理（attack 在 µs~ms 级）；vocal 输入经 AnalysisStrategy 出 RMS/GR 时间线（P1-10）
...
```
                     ┌──────────────────────────────┐
                     │  阶段 2：输入与信号增强        │
                     │  T2.1 FilePlayback(重采样/声道映射/tail pad) │
                     │  T2.2 噪声(白+粉,固定种子)    │
                     │  T2.3 EnvelopeSignal          │
...
**一句话结论**：草案阶段划分和顺序正确，但缺 4 个 P0（采样率重采样、噪声时长语义、GR 干湿对齐、扫描中断/进度）、6 个 P1 实现细节（其中"TestCompressorPlugin 测试设施"是阶段 4 能否 TDD 的前提），按修正后的计划执行即可。
