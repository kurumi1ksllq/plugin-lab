...
**生成:** 2026-08-03 · **规模:** 14 文件（7 分析器 × .h/.cpp + Export.h/.cpp，Export 23 符号为最大）

## OVERVIEW

7 分析器 + Export 层：原始捕获 → 领域结果 → 8 类 schema JSON。
...
| HarmonicAnalysis  | 单音多电平 → THD% + H2..H5 各次谐波 %                                            | 谐波柱状图                             |
...
| CompressionFamily | 输入电平 × 速度网格 → 每格压缩曲线 + GR 时间线                                   | phase 4 交付                           |

### 要点

- **THD/IMD 勿混用**：THD 单音、IMD 多音，混用则谐波峰交叠。
