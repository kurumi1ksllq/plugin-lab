...
1. **测量是手段，理解处理方式是目的** — 反推参数只是过程，最终产出是 AI 对插件"处理链路"的理解
2. **纯黑盒**（贯穿原则 3）— 不依赖插件厂商 API、文档或任何先验知识，只靠注入信号 + 捕获输出
3. **结果可复现** — 测量数据必须能作为后续开发的基准（写自己的 EQ / 压缩器时对照实测行为）
...
**规格→复刻闭环工具链**（2026-08-14 T4 落地中）：`describe_chain.py`（T2，chain_doc `CONTRACT_VERSION="2"`）→ `PluginLabReplica`（规格驱动复刻 VST3，`source/replica/`，由 chain_doc 配置 DSP）→ `compare_all.py`（T3）构成完整闭环——测真机 → 反推 → 描述 → 配置复刻 → 再测 → 对比，验证反推规格可作为开发基准。复刻解析器/DSP/包装已落地，子进程测量测试在途。见 `STATUS.md` T4 记录。
