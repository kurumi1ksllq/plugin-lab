...
| 项目     | 方案                                                                                                                     |
| -------- | ------------------------------------------------------------------------------------------------------------------------ |
| 信号     | **单音正弦（如 1kHz），多电平（-20dB, -10dB, 0dB）测 THD**；**多音测 IMD（互调失真）**，两者分开（现状标签：**IMD 未实现**——当前 `harmonicAnalysis` 仅逐基频 THD，无双音互调测量落地；实现走 issue，本节只定方法）                         |
| 录制     | 湿路                                                                                                                     |
| 分析     | FFT → 基波 + 各次谐波能量                                                                                                |
...
> 注：本节为 2026-08-02 前的历史示例，权威导出契约以 `SPEC.md` 为准（2026-08-11 文档体系定），本节不再同步更新。
